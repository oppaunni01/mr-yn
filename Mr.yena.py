...
cl = MORA.LINE()
#cl.login(qr=True)
cl.login(token='EqLhIKAbo7Emwg55odx9.3UrGMEjECFh9gbFk6EFekq.VNHW8NATh86LdHNZeSMOuR1rSiVUH7ofTH5NetigrY0=')
cl.loginResult()
                            ...
                            if Name in cctv["sidermem"][op.param1]:
                                ...
                                if " " in Name:
                                    ...
                                    if len(nick) == 2:
                                        ...
                                        random.choice(KAC).sendText(op.param1, "Haii " + nick[0] + " Si Tukan Sider Keluar Gak")
                                    else:
                                        time.sleep(0.2)
                                        summon(op.param1,[op.param2])
                                        random.choice(KAC).sendText(op.param1, "Halo " + nick[0] + " Tukan Ngintip Sering Ngintip Tetangga Mandi Ya Kak")
                                else:
                                    time.sleep(0.2)
                                    summon(op.param1,[op.param2])
                                    random.choice(KAC).sendText(op.param1, "Woeee " + nick[0] + " Jangan Jadi Tukan Ngintip Apa Lagi Cari Sasaran Untuk kakak Tikung Ya")
...
#			cl.sendText(op.param1,"☆Ketik ☞Help☜ Untuk Bantuan☆\n☆Harap Gunakan Dengan Bijak ^_^ ☆")
        ...
        if op.type == 17:
          if wait["Sambutan"] == True:
            ...
            cl.sendMessage(c)
            cl.sendText(op.param1,"Haiii " + cl.getContact(op.param2).displayName + "\nWelcome To ☞ " + str(ginfo.name) + " ☜" + "\nNo Baper\nDan Salam sopan santung ^_^")
        ...
        if op.type == 15:
          if wait["Sambutan"] == True:
            if op.param2 in admin:
                return
            cl.sendText(op.param1,"Thanks kakak " + cl.getContact(op.param2).displayName +  "\nUdah Pernah Join Di Room Ini And Maaf Ya Kak Jika Kami Ada Salah (p′︵‵。) 🤗")
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = cl.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["Si Angean Ngetag Aneee","Lagi Di Atas Perut Ojo di Tag",cName + "Woe Ngapain Ngetag?",cName + " Ngetat Mulu Pengen di Angein ya","Napa  Anjirr!","Lagi Anu Ojo di Tag", cName + " Si Dudul","Mbohh \nSi Pea Ntag Mulu " + cName, "Dudul  " + cName, "Ojo Ngetag " + cName + "?", "Ia Ada Apa " + cName + "?","Woii " + cName + " Jones Ye Tag Mulu"]
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
